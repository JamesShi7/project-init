...
- **`/resume`** skill — recover last session's full conversation via jq-filtered JSONL reading
- **`/resume-full`** skill — full project trajectory recovery with historical session timeline
- **Log Compaction Protocol** — auto-compact logs when they exceed 10 files
...
- Added "收工" / "end session" as triggers for full end-of-session protocol
...
- Added "整理文件" as direct trigger for file reorganization
